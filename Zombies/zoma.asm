00000000  EB53              jmp @start_here

00000002  CC                int3
00000003  CC                int3
00000004  CC                int3
00000005  1E                push ds
00000006  07                pop es
00000007  50                push ax
00000008  89C3              mov bx,ax
0000000A  81C3CD00          add bx,0xcd
0000000E  D7                xlatb
0000000F  86D0              xchg dl,al
00000011  58                pop ax
00000012  30E0              xor al,ah
00000014  D7                xlatb
00000015  88D4              mov ah,dl
00000017  89C3              mov bx,ax
00000019  B91000            mov cx,0x10
0000001C  89800001          mov [bx+si+0x100],ax
00000020  D1E3              shl bx,1
00000022  E2F8              loop 0x1c
00000024  46                inc si
00000025  83E63F            and si,byte +0x3f
00000028  EBED              jmp short 0x17
0000002A  CC                int3
0000002B  CC                int3
0000002C  CC                int3
0000002D  1E                push ds
0000002E  07                pop es
0000002F  50                push ax
00000030  89C3              mov bx,ax
00000032  81C3CD00          add bx,0xcd
00000036  D7                xlatb
00000037  86D0              xchg dl,al
00000039  58                pop ax
0000003A  30E0              xor al,ah
0000003C  D7                xlatb
0000003D  88D4              mov ah,dl
0000003F  89C3              mov bx,ax
00000041  B91000            mov cx,0x10
00000044  89800001          mov [bx+si+0x100],ax
00000048  D1E3              shl bx,1
0000004A  E2F8              loop 0x44
0000004C  46                inc si
0000004D  83E63F            and si,byte +0x3f
00000050  EBED              jmp short 0x3f
00000052  CC                int3
00000053  CC                int3
00000054  CC                int3


                            ; _AX -> ax at the start
							; _AL
							; _AH
                            @start_here:
00000055  1E                push ds
00000056  07                pop es ;; es = Arena
00000057  50                push ax
00000058  89C3              mov bx,ax ;; bx = _AX
0000005A  81C3CD00          add bx,0xcd ;; bx = _AX+0xcd
0000005E  D7                xlatb ;; al = [bx + al] = [_AX+0xcd + _AL] ========== i1
0000005F  86D0              xchg dl,al ; dl = [_AX+0xcd + _AL], al = 0
00000061  58                pop ax ; ax = _AX
00000062  30E0              xor al,ah ; al = _AL^_AH
00000064  D7                xlatb ;  al = [bx + al] = [_AX+0xcd + _AL^_AH] ========= i2
00000065  88D4              mov ah,dl ; ah = i1

00000067  89C3              mov bx,ax ; bx = i1i2, ax = i1i2 ;;;;;mov ax,[0x8100]
00000069  B91000            mov cx,0x10						;;;;;;;;
0000006C  89800001          mov [bx+si+0x100],ax ;;;;;;;;;;;;;;;; xchg cx,[0x8100]
00000070  D1E3              shl bx,1;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; cmp ax,cx
00000072  E2F8              loop 0x6c  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;je cmp_again
							;; bx = 0 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;take zomb
00000074  46                inc si
00000075  83E63F            and si,byte +0x3f
00000078  EBED              jmp short 0x67


0000007A  CC                int3
0000007B  CC                int3
0000007C  CC                int3
0000007D  1E                push ds
0000007E  07                pop es
0000007F  50                push ax
00000080  89C3              mov bx,ax
00000082  81C3CD00          add bx,0xcd
00000086  D7                xlatb
00000087  86D0              xchg dl,al
00000089  58                pop ax
0000008A  30E0              xor al,ah
0000008C  D7                xlatb
0000008D  88D4              mov ah,dl
0000008F  89C3              mov bx,ax
00000091  B91000            mov cx,0x10
00000094  89800001          mov [bx+si+0x100],ax
00000098  D1E3              shl bx,1
0000009A  E2F8              loop 0x94
0000009C  46                inc si
0000009D  83E63F            and si,byte +0x3f
000000A0  EBED              jmp short 0x8f
000000A2  CC                int3
000000A3  CC                int3
000000A4  CC                int3
000000A5  1E                push ds
000000A6  07                pop es
000000A7  50                push ax
000000A8  89C3              mov bx,ax
000000AA  81C3CD00          add bx,0xcd
000000AE  D7                xlatb
000000AF  86D0              xchg dl,al
000000B1  58                pop ax
000000B2  30E0              xor al,ah
000000B4  D7                xlatb
000000B5  88D4              mov ah,dl
000000B7  89C3              mov bx,ax
000000B9  B91000            mov cx,0x10
000000BC  89800001          mov [bx+si+0x100],ax
000000C0  D1E3              shl bx,1
000000C2  E2F8              loop 0xbc
000000C4  46                inc si
000000C5  83E63F            and si,byte +0x3f
000000C8  EBED              jmp short 0xb7
000000CA  CC                int3
000000CB  CC                int3
000000CC  CC                int3
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
db 0x00
db 0xbb
db 0xd3
db 0x68
db 0xbe
db 0x05
db 0x6d
db 0xd6
db 0x82
db 0x39
db 0x51
db 0xea
db 0x3c
db 0x87
db 0xef
db 0x54
db 0xe0
db 0x5b
db 0x33
db 0x88
db 0x5e
db 0xe5
db 0x8d
db 0x36
db 0x62
db 0xd9
db 0xb1
db 0x0a
db 0xdc
db 0x67
db 0x0f
db 0xb4
db 0xee
db 0x55
db 0x3d
db 0x86
db 0x50
db 0xeb
db 0x83
db 0x38
db 0x6c
db 0xd7
db 0xbf
db 0x04
db 0xd2
db 0x69
db 0x01
db 0xba
db 0x0e
db 0xb5
db 0xdd
db 0x66
db 0xb0
db 0x0b
db 0x63
db 0xd8
db 0x8c
db 0x37
db 0x5f
db 0xe4
db 0x32
db 0x89
db 0xe1
db 0x5a
db 0xff
db 0x44
db 0x2c
db 0x97
db 0x41
db 0xfa
db 0x92
db 0x29
db 0x7d
db 0xc6
db 0xae
db 0x15
db 0xc3
db 0x78
db 0x10
db 0xab
db 0x1f
db 0xa4
db 0xcc
db 0x77
db 0xa1
db 0x1a
db 0x72
db 0xc9
db 0x9d
db 0x26
db 0x4e
db 0xf5
db 0x23
db 0x98
db 0xf0
db 0x4b
db 0x11
db 0xaa
db 0xc2
db 0x79
db 0xaf
db 0x14
db 0x7c
db 0xc7
db 0x93
db 0x28
db 0x40
db 0xfb
db 0x2d
db 0x96
db 0xfe
db 0x45
db 0xf1
db 0x4a
db 0x22
db 0x99
db 0x4f
db 0xf4
db 0x9c
db 0x27
db 0x73
db 0xc8
db 0xa0
db 0x1b
db 0xcd
db 0x76
db 0x1e
db 0xa5
db 0x80
db 0x3b
db 0x53
db 0xe8
db 0x3e
db 0x85
db 0xed
db 0x56
db 0x02
db 0xb9
db 0xd1
db 0x6a
db 0xbc
db 0x07
db 0x6f
db 0xd4
db 0x60
db 0xdb
db 0xb3
db 0x08
db 0xde
db 0x65
db 0x0d
db 0xb6
db 0xe2
db 0x59
db 0x31
db 0x8a
db 0x5c
db 0xe7
db 0x8f
db 0x34
db 0x6e
db 0xd5
db 0xbd
db 0x06
db 0xd0
db 0x6b
db 0x03
db 0xb8
db 0xec
db 0x57
db 0x3f
db 0x84
db 0x52
db 0xe9
db 0x81
db 0x3a
db 0x8e
db 0x35
db 0x5d
db 0xe6
db 0x30
db 0x8b
db 0xe3
db 0x58
db 0x0c
db 0xb7
db 0xdf
db 0x64
db 0xb2
db 0x09
db 0x61
db 0xda
db 0x7f
db 0xc4
db 0xac
db 0x17
db 0xc1
db 0x7a
db 0x12
db 0xa9
db 0xfd
db 0x46
db 0x2e
db 0x95
db 0x43
db 0xf8
db 0x90
db 0x2b
db 0x9f
db 0x24
db 0x4c
db 0xf7
db 0x21
db 0x9a
db 0xf2
db 0x49
db 0x1d
db 0xa6
db 0xce
db 0x75
db 0xa3
db 0x18
db 0x70
db 0xcb
db 0x91
db 0x2a
db 0x42
db 0xf9
db 0x2f
db 0x94
db 0xfc
db 0x47
db 0x13
db 0xa8
db 0xc0
db 0x7b
db 0xad
db 0x16
db 0x7e
db 0xc5
db 0x71
db 0xca
db 0xa2
db 0x19
db 0xcf
db 0x74
db 0x1c
db 0xa7
db 0xf3
db 0x48
db 0x20
db 0x9b
db 0x4d
db 0xf6
db 0x9e
db 0x25